  ...
  },
  {
    ...
    assert: [
      ...
      a.resultOf(() => ({
        kind: 'select',
        fields: [
          { expr: e.ref('department', 'id').toJSON(), as: 'departmentId' },
        ...
        groupBy: [e.ref('department', 'id').toJSON()],
      })),
    ],
  },
  {
  ...
  },
  {
    ...
    assert: [
      ...
      a.resultOf(() => ({
        ...
        joins: [e.relJoin('payment', 'invoice', 'invoice')],
        where: [
          e.gt(e.age(e.ref('payment', 'paidAt'), e.ref('invoice', 'issuedAt')), e.value(10)).toJSON(),
        ],
      })),
    ],
  },
