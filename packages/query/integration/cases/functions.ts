 ...
 * (`month`, `dayOfWeek`, `age`, `dateTrunc`), and a string builtin (`length`).
 * Each derives its expected answer from the oracle, so a wrong function or the
 * wrong grain diverges.
 */
import { e } from '../model';
...
export const functionCases: EvalCase[] = [
  ...
  },
  {
    ...
    oracle: () => ({
      ...
      where: [
        e.gt(e.age(e.ref('payment', 'paidAt'), e.path('payment', 'invoiceId', 'issuedAt')), e.value(10)).toJSON(),
      ],
    }),
