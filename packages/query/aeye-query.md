...
A `Shape<T>` is a tiny structural schema: `check(json: unknown, ctx): T | INVALID`. It validates the untrusted `json` at the caller's current `Problems` path, RECORDS any problems (reusing `aids.ts` — `aidInfo` for the domain label, `didYouMean` for suggestions — never Zod, never `flattenZodIssues`), and returns the built value or the `INVALID` sentinel. It MUST NOT throw. Combinators: `lit` (a `kind` discriminant), `str` / `num` / `int` / `bool` / `scalar` (typeof / `Number.isInteger` guards → aid-directed `expected <label>, got <received>`), `enumOf` (membership → `expected <label>: a, b, c` + a `didYouMean` tail), `obj` / `optional` / `list` (composites), and `exprRef` (a child-expr slot). **Accumulation, no throwing:** `obj` and `list` check ALL of their fields/elements before deciding — they never early-return on the first bad one — so a single `check` surfaces multiple problems in one pass (e.g. a `comparison` with a bad `op` AND a non-object `left` yields BOTH the `op` enum problem and the `left` not-object problem, each localized under its own path).
...
`Registry.parseCheckedExpr(json, problems)` is the DEFENSIVE dispatch — the zod-free parallel to `parseExpr`: it mirrors the throwing dispatch but records problems (`shape.not-object` / `shape.missing-kind` / `shape.unknown-kind` with a `didYouMean` over the real expr kinds) and returns the built `Expr` or `undefined`, dispatching to each class's owned `static SHAPE`. Exemplar `SHAPE`s live on `comparison`, `field-ref`, `literal`, `param`, and `logical` (whose wrapper adds the `not`-arity=1 rule as an accumulated structural problem); each builds an `Expr` `.toJSON()`-equal to the existing `from(def)` output on a valid def.

**This is the FOUNDATION, not yet the active gate.** Zod still gates `parseQueryInput`; the owned parser is parallel and unit-tested only. Flipping the gate — and extending `SHAPE` to every expr/query kind — is the follow-on step.
