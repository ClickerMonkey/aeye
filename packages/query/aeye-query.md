...
**This is the FOUNDATION, not yet the active gate.** Zod still gates `parseQueryInput`; the owned parser is parallel and unit-tested only. Flipping the gate (C4) — retiring the zod structural authority so `parseCheckedQuery` is the one structural gate feeding `validateWalk` — is the follow-on step.
