...
**Messages are aid-directed (domain-specific), not Zod-generic.** Before: `"left": "oops"` → "expected object, received string"; `"args": "total"` → "expected record, received string"; `"kind": "comparise"` → "Value does not match any of the allowed shapes." After: "expected **an expression**"; "expected **named arguments**, an object of { argName: <expr> }"; "unknown **expression kind** `comparise` — did you mean `comparison`?". Likewise `"op": "equals"` → "expected a comparison operator: =, <>, <, <=, >, >=, like, notLike, ilike"; `"limit": "three"` → "expected a number or a param"; an unknown Type name under an enumerated depth → "expected a registered Type name".
...
### Owned structural parser (zod-free, accumulating) — the foundation

Today two authorities gate a model-authored query: Zod does the STRUCTURAL check (`schema.safeParse`), then the owned `validateWalk` does the SEMANTIC check — and only `validateWalk` has the properties we want: it NEVER throws and ACCUMULATES every problem in one pass. The **`src/shape/` combinator module** is step one of collapsing those two authorities into one, so each expr/query eventually OWNS its shape validation the same way it owns its semantics, and Zod is demoted to the model-facing WIRE schema only.

A `Shape<T>` is a tiny structural schema: `check(json: unknown, ctx): T | INVALID`. It validates the untrusted `json` at the caller's current `Problems` path, RECORDS any problems (reusing `aids.ts` — `aidInfo` for the domain label, `didYouMean` for suggestions — never Zod, never `flattenZodIssues`), and returns the built value or the `INVALID` sentinel. It MUST NOT throw. Combinators: `lit` (a `kind` discriminant), `str` / `num` / `int` / `bool` / `scalar` (typeof / `Number.isInteger` guards → aid-directed `expected <label>, got <received>`), `enumOf` (membership → `expected <label>: a, b, c` + a `didYouMean` tail), `obj` / `optional` / `list` / `record` (composites — `record` is the named-argument map `{ [name]: value }`, returning an insertion-ordered `Map` to mirror `parseNamedArgs`), and `exprRef` (a child-expr slot). **Accumulation, no throwing:** `obj` / `list` / `record` check ALL of their fields/elements before deciding — they never early-return on the first bad one — so a single `check` surfaces multiple problems in one pass (e.g. a `comparison` with a bad `op` AND a non-object `left` yields BOTH the `op` enum problem and the `left` not-object problem, each localized under its own path). Cross-field rules that a plain `obj` can't express (e.g. `logical`'s `not`-arity, or `in`'s list-vs-subquery dispatch) live in a thin hand-written `Shape` wrapper around the base `obj`.
...
`Registry.parseCheckedExpr(json, problems)` is the DEFENSIVE dispatch — the zod-free parallel to `parseExpr`: it mirrors the throwing dispatch but records problems (`shape.not-object` / `shape.missing-kind` / `shape.unknown-kind` with a `didYouMean` over the real expr kinds) and returns the built `Expr` or `undefined`, dispatching to each class's owned `static SHAPE` (it auto-includes every registered class that declares one). Each `SHAPE` builds an `Expr` `.toJSON()`-equal to the existing `from(def)` output on a valid def; the SEMANTIC checks (comparability, arg types, aggregate placement, unknown names) stay in `validateWalk`.

**Expr kinds with a `static SHAPE` today.** The C1 exemplars — `comparison`, `field-ref`, `literal`, `param`, `logical` (whose wrapper adds the `not`-arity=1 rule as an accumulated structural problem) — plus the C2 migration: `binary`, `unary`, `is-null`, `between`, `in` (the LIST form), `case`, `aggregate`, `window`, `function-call`, `relation-path`, `array-op`, `output`, `excluded`, `text-search`, `text-score`. The four function-call kinds share the `record(exprRef())` named-arg map; `array-op`'s polymorphic `value` (single expr / list / omitted) normalizes to `Expr[]`; the two text exprs share a `textQueryShape` (a string literal or a `param`).

**Deferred to C3 (need a `queryRef` / `sourceRef` the shape layer lacks).** `subquery`, `exists`, the `in`-with-**subquery** form (its `SHAPE` records a documented `shape.todo` and bails, leaving the list form intact), `semantic`, `tabular-function-call`, and `filters` keep their throwing `from` only — no `SHAPE` yet — until C3 introduces the query/source reference combinators.

**This is the FOUNDATION, not yet the active gate.** Zod still gates `parseQueryInput`; the owned parser is parallel and unit-tested only. Flipping the gate — and finishing the deferred kinds — is the follow-on step.
