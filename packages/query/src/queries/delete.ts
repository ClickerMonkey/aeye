...
export class DeleteQuery extends Query {
   ...
   * / RETURNING are emitted through the SAME planner so a hidden backing join
   * over the target shares those joins / CTEs. Because the
  ...
  toSQL(dialect: Dialect, ctx: SqlContext): SqlText {
    ...
    // 3. Assemble (planner now holds every USING item / key predicate).
    const parts: SqlText[] = [];
